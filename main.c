...
int main() {
    ...
    int flag = 1; //flag utilizzato per evitare l'indice zero contenuto nel vettore del risultato
//apertura del file
...
//conta il numero totale di righe presenti nel file
    ...
    /*for(int j=0; j<cnt_tot; j++){
        printf(" %s-%s-%s\n", v[j].data.anno, v[j].data.mese, v[j].data.giorno);
    }*/
...
/*
//riordina la struct di vettori letti dal file
